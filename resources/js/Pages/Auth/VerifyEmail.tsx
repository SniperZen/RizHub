...
export default function VerifyEmail({ status }: { status?: string }) {
    ...
    useEffect(() => {
        if (status === "verification-link-sent") {
            setToastMessage("A new verification link has been sent to the email address you provided during registration.");
            setShowToast(true);
    ...
    const submit: FormEventHandler = (e) => {
        ...
        post(route('verification.send'), {
            onSuccess: () => {
                // Show toast immediately when the button is clicked
                setToastMessage("A new verification link has been sent to the email address you provided during registration.");
                setShowToast(true);
                
                // This will be replaced by the status-based toast when the request completes
                const timer = setTimeout(() => {
                    setShowToast(false);
                }, 5000);
                return () => clearTimeout(timer);
        ...
        >
            ...
            <div className="relative z-10 flex max-w-3xl w-full bg-[#FA7816] rounded-2xl shadow-xl overflow-hidden">
                ...
                <div className="hidden md:flex w-1/2 items-center justify-center relative bg-orange-100">
                    ...
                    <h2 className="text-[#FA7816] text-lg md:text-2xl font-extrabold drop-shadow-md leading-snug max-w-xs mt-5">
                        You're now part of the RizHub journey—learn, play, and win with us!
                    </h2>
                    </div>
                </div>
                    ...
                    <div className="w-full">
                        ...
                        <form onSubmit={submit} className="space-y-6">
                            <button
                                type="submit"
                                disabled={processing}
                                className="w-full flex justify-center py-3 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-[#5A3416] hover:bg-[#3d2410] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-75 transition-colors duration-200"
                            >
