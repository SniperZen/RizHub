...
class UserJohnVincentSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        $user = DB::table('users')->where('email', 'johnvincenttejoso@gmail.com')->first();
        
        if (!$user) {
            // Insert new user
            $userId = DB::table('users')->insertGetId([
                'name' => 'John Vincent Tejoso',
                'email' => 'johnvincenttejoso@gmail.com',
        ...
        // Define star distribution for first 5 kabanatas (different patterns)
        $starDistribution = [
            1 => 2,  // Kabanata 1: 2 stars
            2 => 3,  // Kabanata 2: 3 stars
            3 => 2,  // Kabanata 3: 2 stars
            4 => 1,  // Kabanata 4: 1 star
            5 => 3,  // Kabanata 5: 3 stars
        ];
        ...
        for ($i = 1; $i <= 64; $i++) {
            if ($i <= 5) {
                // Kabanatas 1-5: COMPLETED with mixed stars
        ...
        // Insert guess word progress only for 3-star kabanatas (kabanatas 2 and 5)
        $guesswordProgress = [];
        ...
        if (!empty($guesswordProgress)) {
            ...
            $this->command->info("Image gallery unlocked for 3-star kabanatas: " . implode(', ', $imageUnlockedKabanatas));
        } else {
        ...
        $this->command->info('✅ Seeding completed successfully for John Vincent Tejoso!');
        $this->command->info('📊 Progress Summary:');
        $this->command->info('   - Kabanatas 1-5: ✅ COMPLETED (stars: 2, 3, 2, 1, 3)');
        $this->command->info('   - Kabanata 6: 🔓 UNLOCKED but NOT STARTED (0 stars, 0 progress)');
        $this->command->info('   - Kabanatas 7-64: 🔒 LOCKED');
        $this->command->info('   - Image gallery: ✅ UNLOCKED for 3-star kabanatas (Kabanatas 2 & 5)');
        $this->command->info('   - Certificate: ❌ NOT unlocked (need 35+ completed kabanatas)');
        $this->command->info('   - Email: ✅ VERIFIED');
        $this->command->info('   - Quiz Performance: Matches star ratings (3⭐: 100%, 2⭐: 75%, 1⭐: 40%)');
    }
}