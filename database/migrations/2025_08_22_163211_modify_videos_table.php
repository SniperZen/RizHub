...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::create('videos', function (Blueprint $table) {
            $table->unsignedBigInteger('id')->primary(); // Video ID as primary key (not auto-increment)
            $table->string('title');
            $table->string('file_path');
            $table->integer('duration')->nullable();
            $table->unsignedBigInteger('kabanata_id'); // Foreign key to kabanatas
            $table->timestamps();

            // Add foreign key constraint for kabanata_id only
            $table->foreign('kabanata_id')->references('id')->on('kabanatas')->onDelete('cascade');
            
            // REMOVED: $table->foreign('id')->references('id')->on('kabanatas')->onDelete('cascade');
        });
